...
void (*mmu_bank_select_callback)(int from, int to) = ((void*)0);
void (*mmu_bank_config_callback)(void) = ((void*)0);
...
# 45 "../src/memory.c"
    ...
    for (addr = 0; addr < 0x00100000; addr += 256 * 16) {
        ...
        dma_write_to_sram(addr, tmp_buf[0], 256);
        dma_read_from_sram(addr, tmp_buf[1], 256);
        ...
        if (addr == 0)
            continue;
        dma_read_from_sram(0, tmp_buf[1], 256);
        if (memcmp(tmp_buf[0], tmp_buf[1], 256) == 0) {
...
# 1 "../src/../z80/dma_helper.inc" 1
0xD3,0xAD,0x18,0x05,0x00,0x00,0x00,0x00,0x00,0x2A,0x04,0x00,0xED,0x5B,0x06,0x00,
0x3A,0x08,0x00,0x47,0x7E,0x12,0x23,0x13,0x10,0xFA,0x3E,0x00,0x18,0xE2
# 126 "../src/memory.c" 2
    ...
    ((void)((sizeof(params) == 5) || (__assert_fail("sizeof(params) == 5", "../src/memory.c", 142, __func__),0)));
    ((void)((len <= 256) || (__assert_fail("len <= 256", "../src/memory.c", 143, __func__),0)));
    params.src_addr = src;
    ...
    if (src == 0x80) {
        ...
        io_invoke_target_cpu(param_blocks, 3, ((void*)0), 0, bank);
    } else {
        param_blocks[2].addr = buf;
...
{
    uint16_t addr_gap_mask = ~(*board_low_addr_mask_hook)();
    uint16_t addr = (uint16_t)(dest & 0xffff);
    unsigned int n;
    int saved_io_status;
    int bank = ((int)((dest) >> 16));
    ...
    if (!(addr & addr_gap_mask || (addr + len - 1) & addr_gap_mask)) {
        ...
        (*board_write_to_sram_hook)(addr, (uint8_t*)buf, len);
        return;
    }
    io_invoke_target_cpu_prepare(&saved_io_status);
    while (0 < len) {
        n = ((0x80) < (len) ? (0x80) : (len));
        __memcpy_on_target(addr, 0x80, (uint8_t*)buf, n, bank);
        len -= n;
        addr += n;
        buf += n;
    }
...
{
    uint32_t high_addr_mask = (*board_high_addr_mask_hook)();
    ...
    (*board_setup_addrbus_hook)(dest);
    write_to_sram_low_addr(dest, (uint8_t*)buf, len - second_half);

    if (0 < second_half) {
        (*board_setup_addrbus_hook)(dest + len - second_half);
...
static void read_from_sram_low_addr(uint32_t src, void *buf, unsigned int len)
{
    ...
    if (!(addr & addr_gap_mask || (addr + len - 1) & addr_gap_mask)) {
        ...
        (*board_read_from_sram_hook)(addr, (uint8_t*)buf, len);
        return;
    }
    io_invoke_target_cpu_prepare(&saved_io_status);
    while (0 < len) {
        n = ((0x80) < (len) ? (0x80) : (len));
        __memcpy_on_target(0x80, addr, (uint8_t*)buf, n, bank);
        len -= n;
        addr += n;
        buf += n;
    }
...
void dma_read_from_sram(uint32_t src, void *buf, unsigned int len)
{
    ...
    (*board_setup_addrbus_hook)(src);
    read_from_sram_low_addr(src, (uint8_t*)buf, len - second_half);

    if (0 < second_half) {
        (*board_setup_addrbus_hook)(src + len - second_half);
...
void __write_to_sram(uint32_t dest, const void *buf, unsigned int len)
{
    (*board_setup_addrbus_hook)(dest);
    (*board_set_data_dir_hook)(0x00);
    (*board_write_to_sram_hook)(dest & 0xffff, (uint8_t*)buf, len);
}
...
void __read_from_sram(uint32_t src, const void *buf, unsigned int len)
{
    (*board_setup_addrbus_hook)(src);
    (*board_set_data_dir_hook)(0xff);
    (*board_read_from_sram_hook)(src & 0xffff, (uint8_t*)buf, len);
}
