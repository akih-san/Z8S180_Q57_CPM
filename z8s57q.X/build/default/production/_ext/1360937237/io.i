...
# 1 "C:\\Program Files\\Microchip\\xc8\\v2.41\\pic\\include\\c99\\assert.h" 1 3
# 12 "C:\\Program Files\\Microchip\\xc8\\v2.41\\pic\\include\\c99\\assert.h" 3
# 1 "C:\\Program Files\\Microchip\\xc8\\v2.41\\pic\\include\\c99\\bits/assert.h" 1 3
# 19 "C:\\Program Files\\Microchip\\xc8\\v2.41\\pic\\include\\c99\\bits/assert.h" 3
          void __assert_fail (const char *, const char *, int, const char *);
# 13 "C:\\Program Files\\Microchip\\xc8\\v2.41\\pic\\include\\c99\\assert.h" 2 3
...
static uint8_t hw_ctrl_lock = 0xff;
...
char getch_buffered(void) {
    ...
    while (1) {
        ...
        GIE = 1;
        _delay((unsigned long)((1000)*(64000000UL/4000000.0)));
    }
...
int cpm_disk_read(unsigned int drive, uint32_t lba, void *buf, unsigned int sectors)
{
    ...
    if (num_drives <= drive || drives[drive].filep == ((void*)0)) {
    ...
    FRESULT fres;
    if ((fres = f_lseek(filep, lba * 128)) != FR_OK) {
        printf("f_lseek(): ERROR %d\n\r", fres);
        return -1;
    }
    while (result < sectors) {
        if ((fres = f_read(filep, buf, 128, &n)) != FR_OK || n != 128) {
            printf("f_read(): ERROR res=%d, n=%d\n\r", fres, n);
            return result;
        }
        buf = (void*)((uint8_t*) + 128);
        lba++;
        result++;
    }
    return result;
...
void try_to_invoke_monitor(void) {
        ...
        (*board_set_busrq_pin_hook)(0);
        _delay((unsigned long)((20)*(64000000UL/4000000.0)));
        if ((*board_io_event_hook)()) {

            (*board_set_busrq_pin_hook)(1);
...
void io_handle() {
    ...
    int do_bus_master = 0;
    uint8_t io_addr = (*board_addr_l_pins_hook)();
    uint8_t io_data = (*board_data_pins_hook)();

    if ((*board_rd_pin_hook)()) {
    ...
    (*board_set_data_dir_hook)(0x00);
    switch (io_addr) {
    case 0x40:
        if (key_input) {
            (*board_set_data_pins_hook)(0xff);
        } else {
            (*board_set_data_pins_hook)(0x00);
        }
        break;
    case 0x41:
        con_flush_buffer();
        c = getch_buffered();
        (*board_set_data_pins_hook)(c);
        break;
    case 0x48:
        if (disk_datap && (disk_datap - disk_buf) < 128) {
            (*board_set_data_pins_hook)(*disk_datap++);
        } else
        if ((debug.disk || debug.disk_read || debug.disk_write || debug.disk_verbose)) {
        ...
        break;
    case 0x4e:
        (*board_set_data_pins_hook)(disk_stat);
        break;
    case 160:
        (*board_set_data_pins_hook)(hw_ctrl_read());
        break;
    default:
    ...
    (*board_set_busrq_pin_hook)(0);
    (*board_clear_io_event_hook)();
    (*board_set_wait_pin_hook)(1);
    while(!(*board_ioreq_pin_hook)());
    (*board_set_data_dir_hook)(0xff);
 ...
 io_write:
    ...
    case 0x48:
        if (disk_datap && (disk_datap - disk_buf) < 128) {
            *disk_datap++ = io_data;
            if (3 == 3 && (disk_datap - disk_buf) == 128) {
                do_bus_master = 1;
            }
        } else
        if ((debug.disk || debug.disk_read || debug.disk_write || debug.disk_verbose)) {
    ...
    case 0x4d:
        ...
        if (((debug.disk_read) && (disk_op == 0 || disk_op == 2 )) ||
            ((debug.disk_write) && (disk_op == 1 || disk_op == 3))) {
            if ((debug.disk_verbose) && !(debug.disk_mask & (1 << disk_drive))) {
 ...
 enter_bus_master:
    ...
    case 172:
        ...
        while (!mon_step_execution && mon_prompt() != 1);
        mon_leave();
        io_stat_ = IO_STAT_INTERRUPTED;
        goto exit_bus_master;
    case 170:
    ...
    if (disk_op == 0 || disk_op == 2) {
        ...
        if ((debug.disk_read) && (debug.disk_verbose) && !(debug.disk_mask & (1 << disk_drive))) {
        ...
        if (disk_op == 0) {
            ...
            dma_write_to_sram(addr, disk_buf, 128);
            disk_datap = ((void*)0);
# 564 "../src/io.c"
        } else {
    ...
    if (disk_op == 1 || disk_op == 3) {
        ...
        if (disk_op == 1) {
            ...
            uint32_t addr = ((uint32_t)mmu_bank << 16) | ((uint16_t)disk_dmah << 8) | disk_dmal;
            dma_read_from_sram(addr, disk_buf, 128);
        } else {
...
int io_wait_write(uint8_t wait_io_addr, uint8_t *result_io_data)
{
    ...
    ((void)((io_stat() == IO_STAT_NOT_STARTED || io_stat() == IO_STAT_STOPPED || io_stat() == IO_STAT_INTERRUPTED || io_stat() == IO_STAT_PREPINVOKE || io_stat() == IO_STAT_MONITOR) || (__assert_fail("io_stat() == IO_STAT_NOT_STARTED || io_stat() == IO_STAT_STOPPED || io_stat() == IO_STAT_INTERRUPTED || io_stat() == IO_STAT_PREPINVOKE || io_stat() == IO_STAT_MONITOR", "../src/io.c", 655, __func__),0)));
    ...
    while (1) {
        ...
        (*board_wait_io_event_hook)();
        if (!(*board_io_event_hook)())
            continue;

        io_addr = (*board_addr_l_pins_hook)();
        io_data = (*board_data_pins_hook)();

        if ((*board_rd_pin_hook)() == 0 && io_addr == 0x4e) {
# 678 "../src/io.c"
            (*board_set_busrq_pin_hook)(0);
            (*board_set_data_dir_hook)(0x00);
            (*board_set_data_pins_hook)(disk_stat);
        ...
        if ((*board_rd_pin_hook)() == 0 || (io_addr != 0x41 && io_addr != wait_io_addr)) {

            printf("%s: ERROR: I/O %5s %3d, %3d (%02XH, %02XH) while waiting for %d (%02XH)\n\r",
                   __func__, (*board_rd_pin_hook)() == 0 ? "read" : "write",
...
# 1 "../src/../z80/dummy.inc" 1
0x3E,0x00,0xD3,0xAD,0x76,0x18,0xF9
# 729 "../src/io.c" 2

    };

    ((void)((io_stat() == IO_STAT_NOT_STARTED || io_stat() == IO_STAT_STOPPED || io_stat() == IO_STAT_MONITOR) || (__assert_fail("io_stat() == IO_STAT_NOT_STARTED || io_stat() == IO_STAT_STOPPED || io_stat() == IO_STAT_MONITOR", "../src/io.c", 733, __func__),0)));
    ...
    *saved_status = io_stat();
    ...
    if (io_stat() == IO_STAT_NOT_STARTED) {
        ...
        bus_master(1);
        __write_to_sram((((uint32_t)(0) << 16) + (0x00000)), dummy_rom, sizeof(dummy_rom));
        (*board_start_z80_hook)();
        (*board_set_bank_pins_hook)((((uint32_t)(0) << 16) + (0x00000)));
        io_wait_write(173, ((void*)0));
    }
    ...
    io_wait_write(171, ((void*)0));




    mon_prepare();
    io_wait_write(172, ((void*)0));
                         ...
                         const param_block_t *outparams, unsigned int noutparams, int bank)
{
    ...
    ((void)((io_stat() == IO_STAT_MONITOR) || (__assert_fail("io_stat() == IO_STAT_MONITOR", "../src/io.c", 779, __func__),0)));
    mon_use_zeropage(bank);

    for (i = 0; i < ninparams; i++) {
        __write_to_sram((((uint32_t)(bank) << 16) + (inparams[i].offs)), inparams[i].addr,
                        inparams[i].len);
    }


    io_wait_write(173, &result_data);

    for (i = 0; i < noutparams; i++) {
        __read_from_sram((((uint32_t)(bank) << 16) + (outparams[i].offs)), outparams[i].addr,
                         outparams[i].len);
    }
...
void io_invoke_target_cpu_teardown(int *saved_status)
{
    ...
    ((void)((io_stat() == IO_STAT_MONITOR) || (__assert_fail("io_stat() == IO_STAT_MONITOR", "../src/io.c", 804, __func__),0)));
    ...
    if (*saved_status == IO_STAT_NOT_STARTED) {
        (*board_set_reset_pin_hook)(0);
        bus_master(1);
        io_stat_ = IO_STAT_NOT_STARTED;
    }

    *saved_status = IO_STAT_INVALID;
}
